...
const files = ['seekAdsStore.js', 'parseEvent.js'];
const funcRoot = path.join(root, 'functions');
const seedData = path.join(root, 'data', 'seek-ads.json');

const dirs = fs
  .readdirSync(funcRoot, { withFileTypes: true })
  .filter((d) => d.isDirectory() && d.name.startsWith('sa-'))
...
for (const dir of dirs) {
  ...
  if (fs.existsSync(seedData)) {
    ...
    fs.copyFileSync(seedData, path.join(dataDir, 'seek-ads.json'));
  }
}
