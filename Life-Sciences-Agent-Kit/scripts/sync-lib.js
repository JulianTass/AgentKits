...
const files = ['lifeSciencesStore.js', 'parseEvent.js'];
const funcRoot = path.join(root, 'functions');
const seedData = path.join(root, 'data', 'life-sciences.json');

const dirs = fs
  .readdirSync(funcRoot, { withFileTypes: true })
  .filter((d) => d.isDirectory() && d.name.startsWith('ls-'))
...
for (const dir of dirs) {
  ...
  if (fs.existsSync(seedData)) {
    ...
    fs.copyFileSync(seedData, path.join(dataDir, 'life-sciences.json'));
  }
}
